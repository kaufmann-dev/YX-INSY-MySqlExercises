...
with report as(
select I.ingredient_id, I.label, count(DIJ.ingredient_id) count
from DISHES D join DISH_INGREDIENTS_JT DIJ on D.dish_id = DIJ.dish_id join INGREDIENTS I on DIJ.ingredient_id = I.ingredient_id
group by I.ingredient_id)
select ingredient_id, label from report h1
where not exists(select * from report h2 where h2.count > h1.count);;
...
select ORDER_ID, round((COS.STATUS_CHANGED_AT - COS.STATUS_CHANGED_AT)*60*24)
from CUSTOMER_ORDER_STATUS_JT COS
group by COS.STATUS_CHANGED_AT;
